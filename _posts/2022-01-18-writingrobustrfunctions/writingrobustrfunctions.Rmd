...
Note that here, we add three `if` statements, one for each required
argument. We must do this if we want to display specific error messages
for each argument. If we want to display a single error message, we can
do so by clubbing the `missing` functions inside an `any` which will
