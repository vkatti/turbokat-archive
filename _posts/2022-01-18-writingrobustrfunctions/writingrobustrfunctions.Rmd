...
In this case, the function works because the arguments when combined
into a single string using `paste` matches the `format` argument of the
`as.Date` function in the main logic of `make_date` which is
`as.Date(paste(yyyy, mm, dd, sep = "-"), format = "%Y-%m-%d")`
...
We can see that the function works with character inputs. In the first
instance above, it gives correct result but gives `NA` for the second.
