...
As you can see from above examples, `factor` do get converted to numeric
but do not give the right results. So we can safely say that factors are
not number-like.

### Dataypes check continued...
...
make_date <-  function(yyyy, mm = 1, dd = 1) {
  ...
  # Check data types
  if (!inherits(yyyy, c("numeric","character"))) {
    stop("argument `yyyy` must be numeric")
  } else if (inherits(yyyy, "character") & is.na(as.numeric(yyyy))) {
    stop("argument `yyyy` must be numeric")
  }
  if (!inherits(mm, c("numeric","character"))) {
    stop("argument `mm` must be numeric")
  } else if (inherits(mm, "character") & is.na(as.numeric(mm))) {
    stop("argument `mm` must be numeric")
  }
  if (!inherits(dd, c("numeric","character"))) {
    stop("argument `dd` must be numeric")
  } else if (inherits(dd, "character") & is.na(as.numeric(dd))) {
    stop("argument `dd` must be numeric")
  }
...
is_numberlike <- function(x){
  ...
  } else if (inherits(x, "character") & is.na(as.numeric(x))) {
...
A special scenario here is when input type is `factor` or `boolean`.
Let's look at how our function behaves with `factor` data type.
...
The function works with `factor` data type because the `factor` function
converts the input into `character` type internally.

```{r}
factor("2022")
paste(factor("2022"))
as.numeric(factor("2022"))
```
...
and for arguments with incorrect sizes.
