...
![Continent-wise Average Life Expectancy over last 2 years](images/Step1.png)
...
`pivot_wider()` creates unique combinations of all columns not included in `names_from` or `values_from` argument. Therefore, if your dataframe/tibble had a primary key prior to the transformation, the primary key of your transformed "wide" dataframe is your old primary key + unique combinations of all columns not included in `names_from` or `values_from` argument. This gives an unusable output with `NA`s. To specify which column/s to be made unique, pass their name to the `id_cols` argument.
